...
namespace hob {
    ...
    uint32_t CameraComponent::get_pixels_per_meter() const {
        return m_pixels_per_meter;
    }

    void CameraComponent::set_pixels_per_meter(uint32_t value) {
        m_pixels_per_meter = value;
    }

    float CameraComponent::get_effective_pixels_per_meter() const {
        return static_cast<float>(m_pixels_per_meter) * m_zoom;
    }
    ...
    Matrix4x4 CameraComponent::build_view_projection() const {
        ...
        const float ppm = get_effective_pixels_per_meter();
    ...
    Vector2 CameraComponent::world_to_screen(const Vector2& world_pos, const Vector2& camera_pos) const {
        ...
        Vector2 delta_pixels = delta_meters * get_effective_pixels_per_meter();
    ...
    Vector2 CameraComponent::screen_to_world(const Vector2& screen_pos, const Vector2& camera_pos) const {
        ...
        const Vector2 delta_meters = delta_pixels / get_effective_pixels_per_meter();
